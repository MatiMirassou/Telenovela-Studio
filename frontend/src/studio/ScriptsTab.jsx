...
export default function ScriptsTab({ project, projectId }) {
  ...
  const loadData = async () => {
    try {
      const eps = await api.getEpisodes(projectId);
      setEpisodes(eps);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const generateBatch = async () => {
    setGenerating(true);
    try {
      const result = await api.generateEpisodes(projectId, 5);
      await loadData();
    ...
    <div className="tab-content">
      ...
      <div className="progress-section">
        <div className="progress-info">
          ...
          <div className="progress-bar large">
            <div className="progress-fill" style={{ width: `${(generatedCount / totalEpisodes) * 100}%` }}></div>
          </div>
        </div>
        {generatedCount < totalEpisodes && (
          <button onClick={generateBatch} disabled={generating} className="btn btn-primary">
      ...
      {episodes.length > 0 && (
        <div className="episodes-grid">
          {episodes.map((ep) => (
            <div key={ep.id} className={`episode-card card ${ep.state}`} onClick={() => ep.state !== 'pending' && viewEpisode(ep)}>
              ...
              {ep.cliffhanger_moment && <p className="cliffhanger">{ep.cliffhanger_moment}</p>}
            </div>
          ))}
            ...
            <div className="modal-header">
              ...
              <div className="modal-header-actions">
                <div className="view-toggle">
                  <button className={`btn btn-sm ${viewMode === 'script' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setViewMode('script')}>Script View</button>
                  <button className={`btn btn-sm ${viewMode === 'data' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setViewMode('data')}>Data View</button>
                </div>
